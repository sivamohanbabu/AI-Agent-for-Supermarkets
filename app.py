...
def render_metric_cards(inventory: pd.DataFrame, rescue: pd.DataFrame) -> None:
    ...
    from agents.rescue_agent import calculate_sustainability_impact
    impact = calculate_sustainability_impact(rescue)
    co2_saved = impact["co2_saved_kg"]

    cols = st.columns(5)
    cards = [
        metric_tile("Total Products", f"{total_products:,}", "Unique SKUs in the file"),
        ...
        metric_tile("Waste Risk Score", f"{risk_score}/100", "Spillage risk indicator"),
        metric_tile("CO2 Prevented", f"{co2_saved:,} kg", "Saved via rescue campaigns"),
    ]
...
def generate_printable_audit_report(report_df: pd.DataFrame, co2_saved: float, water_saved: float) -> str:
    today_str = date.today().strftime("%B %d, %Y")
    expired_items = report_df[report_df["expiry_status"] == "Expired"]
    expired_count = len(expired_items)
    critical_items = report_df[report_df["expiry_status"] == "Critical"]
    critical_count = len(critical_items)
    
    md = f"""# Smart Food Waste Reduction - System Audit Report
**Date Generated:** {today_str}
**Compliance & Safety Status:** Grounded on FSSAI guidelines and supermarket policy SOPs.

---

## 🍃 Sustainability Impact & Storytelling
- **Carbon Footprint Prevented:** {co2_saved:,} kg of CO2 equivalent emissions saved.
- **Water Saved:** {water_saved:,} Liters of freshwater footprint saved.
- **NGO Donations Recommended:** {len(report_df[report_df["food_rescue_action"] == "NGO Donation"])} items scheduled.

## 🚨 Operations Summary
- **Expired Items (Remove Immediately):** {expired_count} products flagged for disposal.
- **Critical Expiry Items (Rescue Needed):** {critical_count} products expiring in <= 3 days.

---

## 📋 Actionable Product Guidance
| Product Name | Current Stock | Forecast Demand (7d) | Reorder Qty | Expiry Status | Discount | Rescue Action |
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
"""
    for _, row in report_df.iterrows():
        md += f"| {row['product_name']} | {row['current_stock']} | {row['forecasted_demand']} | {row['recommended_reorder']} | {row['expiry_status']} | {row['recommended_discount_pct']}% | {row['food_rescue_action']} |\n"
        
    md += """
---
*Disclaimer: Generated by the Smart Food Waste Reduction AI Agent. All suggestions are guided by the offline store safety database.*
"""
    return md
...
def main() -> None:
    ...
    with dashboard_tab:
        render_metric_cards(inventory, rescue)
        section_title(
    ...
    with ai_tab:
        ...
        with report_col:
            ...
            # Sustainability & Compliance Audit Report Exporter
            from agents.rescue_agent import calculate_sustainability_impact
            impact = calculate_sustainability_impact(report)
            md_content = generate_printable_audit_report(report, impact["co2_saved_kg"], impact["water_saved_liters"])
            st.download_button(
                "📄 Download Printable Audit Report (Markdown)",
                data=md_content,
                file_name="supermarket_food_waste_audit.md",
                mime="text/markdown",
                use_container_width=True,
            )
            st.markdown("---")
